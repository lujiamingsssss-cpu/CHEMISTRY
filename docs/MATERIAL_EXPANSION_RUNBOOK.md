...
> 本手册记录首次真实扩展成功后可重复执行的操作流程。产品与安全边界见 `PROJECT_SPEC.md`，仓库级工程规则见根目录 `AGENTS.md`；本手册不能扩大两者的范围。
...
全部满足后，只在长期产品边界确有变化时更新 `PROJECT_SPEC.md`；任务状态、完成清单和交接快照不进入永久文档。按授权提交并推送代码、测试、清单和必要运行手册变更，然后立即停止，不自动开始下一阶段。
