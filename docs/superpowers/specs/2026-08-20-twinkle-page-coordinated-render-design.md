...
### 8.7 阶段 5：首页受控基线、单状态机与 H0–H3 门禁

#### 8.7.1 授权范围与阶段边界

阶段 5 的目标是在受 Git 管理的 `showcase/homepage` 建立可独立运行的首页候选，并在后续人工门禁通过后，把阶段 4 已关闭的 C360、四条聚焦路线和阶段 3 r2 机械动作接入同一受控视窗。阶段四的 `authorizesStage5=false` 保持历史事实不变；阶段 5 的每次授权由本节的独立人工门禁记录，不能回写或改称阶段四曾授权阶段五。

Milestone 1 只建立首页基线、静态 TWINKLE 入口、正式运行时资产和资产权威。静态入口固定文案为“技术能力展示样机”“TWINKLE 开放光学系统”“在同一受控视窗中总览 TWINKLE，并沿四条已批准路线查看双通道采集光学舱与聚光镜组件的结构动作。”“360° 结构探索”“2 个结构单元 · 4 条聚焦路线”“点击打开视窗 →”。首次入视口只执行一次 700 ms 上移归位；hover 放大不超过 1%，阴影略深，箭头右移 4 px；减少动态时完全静态。Milestone 1 不创建浏览器 runtime manifest，不开发播放、路线、讲解或状态机。

后续阶段 5 交互只能采用**单一状态机**：同一个状态权威协调总览捕获帧、cyclic 转向、获批聚焦路线、阶段 3 机械展开/检查灯、讲解态、退出交接、反序闭合和 `overviewReturn`。不得让 C360、路线、机械动作或讲解面板各自维护第二套互相竞争的状态，也不得恢复 J/F 或滤片状态。

#### 8.7.2 正式资产契约

阶段 5 的唯一运行时资产权威为普通 Git 文件 `registry/twinkle/stage5-runtime-assets.json`。它必须机械生成并绑定 Milestone 0 dry-run、来源分支与 HEAD、七份原始权威 manifest 的逐字节副本、每个来源/目标路径、角色、路线、SHA-256 和字节数；不得手工维护第二套清单。受控目标只包含 248 张 PNG、准确总量 130,156,891 bytes：C360 96 张，`006-A`、`065-B`、`087-A`、`008-A` 四条最终路线各 25 张，以及阶段 3 机械帧和检查灯 52 张。

所有正式 PNG 只允许匹配 `showcase/homepage/assets/twinkle/**/*.png` 的 Git LFS 规则。不得扩大为全仓 `*.png`，不得迁移另外四条候选路线、联系表、path-speed 图、审核截图、浏览器原始结果、诊断资产、`.blend`、历史候选或可重建中间产物。迁移必须拒绝既有目标、路径逃逸、reparse point、缺失/额外文件、哈希或字节数漂移和未物化 LFS pointer，并在同卷任务独占 staging 完整验证后以目录原子 rename 发布；来源工作树和原 `artifacts` 始终只读。

#### 8.7.3 人工审核点

- **H0**：核对 LFS 存储/带宽与准确资产规模，只能授权 Milestone 1。2026-08-31 用户在仓库所有者 `lujiamingsssss-cpu` 的 GitHub Usage 页面确认当月 `git_lfs_storage` 与 `git_lfs_bandwidth` 均为 `No usage found`，并据 10 GiB/10 GiB 包含量批准本轮 0.121218 GiB 的 Milestone 1；不授权远程 LFS 写入或后续里程碑。
- **H1**：审核受控首页与原首页桌面/移动对照、非 `#quality` 漂移、本地依赖闭包、外部链接、静态入口动效、248 文件权威、LFS 物化、来源不变、测试和回滚。H1 明确批准前不得创建 `twinkle-runtime-manifest.json` 或进入 Milestone 2。
- **H2**：只审核 H1 后另行批准的单一状态机、C360 播放、四条路线与失败/减少动态契约；不得由 H1 自动授权，也不得提前接入讲解内容或阶段 6。
- **H3**：审核阶段 5 完整双单元讲解、机械/检查灯交接、退出与回总览、无障碍和最终浏览器证据。只有 H3 明确批准才能关闭阶段 5，并提交阶段 6 的独立设计/集成审批。

任一机器测试、历史浏览器证据或前一人工门禁均不得代签下一门禁。所有 H0–H3 授权都不授权提交、push、PR、部署或线上覆盖；这些操作始终需要独立明确授权。

#### 8.7.4 阶段 6 边界

阶段 6 只能集成 H3 已批准的阶段 5 候选，不得在集成时重做视觉、改写资产权威、增加热点/路线、引入第二状态机或改变阶段 1–4 的批准事实。阶段 6 的生产接入、发布、线上覆盖与回滚必须另行设计、验证和人工批准；阶段 5 关闭本身不构成任何发布授权。
