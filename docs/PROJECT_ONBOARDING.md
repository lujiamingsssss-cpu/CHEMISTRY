...
**跨工作树预览**：`--root <目录>` 可指定服务根目录。这在本项目很关键——TWINKLE 完整流程的产物属证据层（刻意不入 Git），只存在于产出它的那个工作树。若首页入口点了变黑屏，多半是服务根目录里没有该流程页（`/output/twinkle-stage5-h2-full-flow-review/index.html` 返回 404）；改用 `--root` 指向持有这些产物的 worktree 即可完整走通。
